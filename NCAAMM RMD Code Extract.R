...
## ----call_reference_season_function--------------------------------------
...
## ----make_pure_naive_entries, cache=TRUE---------------------------------
my_pure_naive_entries <- create_pure_naive_entries(1000)

## ----display_pure_naive_format-------------------------------------------
my_pure_naive_entries[[1]]

## ----score_entries_function----------------------------------------------
...
names(my_palette) <- c("SeaGreen", "DarkPurple", "Acajou", "AntiqueBrass", "Flax")

backgrounds <- c("#FBF8F1", "#EAEFFA", "#393732")
show_col(my_palette, labels = TRUE)
cols <- c("pure-naive" = my_palette[5], "smart_naive" = my_palette[2], "historic" = my_palette[4])

model_type <- factor(c("pure-naive", "smart_naive", "historic"))

ggplot(my_pure_naive_results, aes(x=total_points)) +
  geom_density(aes(alpha = 0.5, fill = model_type[1])) + 
...
## ----create_smart_naive_entries_function---------------------------------
...
## ----call_create_smart_naive_function, cache=TRUE------------------------
my_smart_naive_entries <- create_smart_naive_entries(1000)

## ----score_smart_naive_entries, cache=TRUE-------------------------------
my_smart_naive_results <- score_entries(my_smart_naive_entries, reference_season)

## ----summarize_smart_naive_scores----------------------------------------
summary(my_smart_naive_results$total_points)
(this_sd <- sd(my_smart_naive_results$total_points))

## ----plot_smart_naive_distribution---------------------------------------
#model_type <- c("pure-naive", "smart_naive", "historic")
library(RColorBrewer)
names(cols) <- c("pure-naive", "smart-naive", "historic")
ggplot(my_smart_naive_results, aes(x=total_points, fill = cols[2],  color = model_type[2])) +
  geom_density(aes(alpha = 0.5)) + 
  geom_vline(aes(xintercept = mean(total_points), 
                 color = model_type[2])) +
  #scale_color_brewer(palette = "Set2") + 
  #scale_fill_manual(values = model_type[2]) +
  theme(legend.position = "none") + 
  ggtitle("Smart-Naive entry performance")
...
## ----create_historic_entries_function------------------------------------
...
combined_results$model <- factor(combined_results$model)

ggplot(combined_results, aes(x=total_points)) +
  geom_density(aes(alpha = 0.5, fill = model)) + 
  geom_vline(aes(xintercept = mean(total_points), 
                 color = my_palette[2])) +
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) 
...
## ----create_reference_season_list_function-------------------------------
...
## ----get_reference_season_list, cache=TRUE-------------------------------
reference_season_list <- create_reference_season_list()

## ----display_reference_season_format_2-----------------------------------
reference_season_list[[1]]

## ----score_entry_lists_function, cache=TRUE------------------------------
...
## ----score_all_entries_all_years, eval=FALSE-----------------------------
scored_entries <- score_entry_lists(all_entries)

## ----scoring_placeholder, cache=TRUE-------------------------------------
scored_entries <- readRDS("input/scored_entries_v3.RDS")
...
p <- ggplot(scored_entries, aes(x=total_points, fill = model, color = NULL)) +
  ...
  geom_label(aes(x = avg_points + 25, y = .040, 
                label = paste("Average points:", format(avg_points, digits = 2)))) +
  theme_solarized()
ggplotly(p)
